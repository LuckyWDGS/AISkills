...
默认顺序是效果第一、性能第二。先生成或选择能匹配目标效果的纹理，优先使用 2 的幂次方工作尺寸，例如 `512x512`、`1024x1024`、`2048x2048`；效果成立后再做降采样、通道打包、压缩格式、LOD、mip、质量开关或低配版本。不要因为库里有一张更便宜的通用纹理，或者因为初始预算紧，就把参考图里的视觉身份改没。
