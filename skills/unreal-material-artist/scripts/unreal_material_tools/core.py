...
MATERIAL_DIR_NAME = "material-delivery"
...
def resolve_root_context(root: str | Path | None = None) -> RootContext:
    ...
    material_root = ensure_dir(session_root / MATERIAL_DIR_NAME)
    return RootContext(
...
def default_report_path(ctx: RootContext, category: str, effect: str, stem: str, suffix: str) -> Path:
    return ctx.material_root / category / slugify(effect) / f"{slugify(stem)}{suffix}"
