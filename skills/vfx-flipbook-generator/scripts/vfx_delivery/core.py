...
def normalize_cli_global_args(
    argv: list[str] | None,
    *,
    known_subcommands: set[str],
    global_opts_with_value: set[str] | None = None,
    global_flag_opts: set[str] | None = None,
) -> list[str] | None:
    if argv is None:
        argv = sys.argv[1:]
    global_opts_with_value = global_opts_with_value or {"--root", "--project", "--endpoint", "--timeout"}
    global_flag_opts = global_flag_opts or set()
    args = list(argv)
    if not args:
        return args
    command_index = next((idx for idx, token in enumerate(args) if token in known_subcommands), None)
    if command_index is None:
        return args
    head = args[:command_index]
    ...
    while idx < len(tail):
        token = tail[idx]
        if token in global_opts_with_value and idx + 1 < len(tail):
            moved.extend([token, tail[idx + 1]])
            idx += 2
            continue
        if token in global_flag_opts:
            moved.append(token)
            idx += 1
            continue
        kept_tail.append(token)
        idx += 1
    return [*head, *moved, args[command_index], *kept_tail]


def normalize_cli_global_args_no_subcommand(
    argv: list[str] | None,
    *,
    global_opts_with_value: set[str] | None = None,
    global_flag_opts: set[str] | None = None,
    ...
    while idx < len(args):
        token = args[idx]
        if token in global_opts_with_value and idx + 1 < len(args):
            moved.extend([token, args[idx + 1]])
            idx += 2
            continue
        if token in global_flag_opts:
            moved.append(token)
