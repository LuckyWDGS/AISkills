...
def pad_power_of_two(value: int) -> int:
    if value <= 0:
        raise ValueError("value must be greater than 0")
...
def atlas_size_for_mode(raw_size: tuple[int, int], mode: str) -> tuple[int, int]:
    ...
    if mode == "pad-to-power-of-two":
        return pad_power_of_two(raw_size[0]), pad_power_of_two(raw_size[1])
    raise ValueError(f"Unknown atlas size mode: {mode}")


def infer_asset_descriptor(effect_name: str, video_stem: str) -> str:
    text = f"{effect_name} {video_stem}".strip()
    lowered = text.lower()
    dust_like = any(token in text for token in ("灰尘", "粉尘")) or "dust" in lowered
    falling_like = "下落" in text or any(token in lowered for token in ("fall", "falling"))
    smoke_like = "烟" in text or "smoke" in lowered
    fire_like = "火" in text or any(token in lowered for token in ("fire", "flame"))
    ember_like = "余烬" in text or "embers" in lowered or "ember" in lowered
    spark_like = "火花" in text or "sparks" in lowered or "spark" in lowered

    if dust_like and falling_like:
        return "DustFall"
    if dust_like:
        return "Dust"
    if smoke_like:
    ...
    for token in ascii_tokens:
        ...
        if lowered_token in GENERIC_ASSET_TOKENS:
            continue
        if re.fullmatch(r"\d+", lowered_token):
            continue
        if re.fullmatch(r"\d+x\d+", lowered_token):
            continue
        filtered.append(token)
    if filtered:
        return "".join(part[:1].upper() + part[1:] for part in filtered)
    return "FlipbookAtlas"


def build_atlas_name(effect_name: str, video_path: Path, columns: int, rows: int) -> str:
    descriptor = infer_asset_descriptor(effect_name, video_path.stem)
    return f"T_{descriptor}_{columns}x{rows}_VFX.png"
...
def build_recommendation(
    ...
    *,
    ...
    max_texture_size: int,
    allow_upscale: bool,
    atlas_size_mode: str = "pad-to-power-of-two",
) -> dict[str, Any]:
    if target_fps <= 0:
    ...
    grid_divides_estimated = estimated_atlas_size[0] % columns == 0 and estimated_atlas_size[1] % rows == 0
    ...
    return {
        ...
        "estimated_grid_divides_atlas": grid_divides_estimated,
        "estimated_cell_size_exact": (
            {"width": estimated_atlas_size[0] // columns, "height": estimated_atlas_size[1] // rows}
            if grid_divides_estimated
            else None
        ),
        "max_texture_size": max_texture_size,
        ...
        "ue_notes": {
            ...
            "grid_divides_atlas": grid_divides_estimated,
            "sub_uv_direct_ready": grid_divides_estimated
            and is_power_of_two(estimated_atlas_size[0])
            and is_power_of_two(estimated_atlas_size[1]),
            "sub_uv_risk": None
            if grid_divides_estimated
            else "estimated atlas dimensions do not divide evenly by grid; direct UE SubUV can jitter or sample offset",
        },
    }
...
def build_command(args: argparse.Namespace) -> int:
    ...
    atlas_name = args.atlas_name or build_atlas_name(effect, video_path, columns, rows)
    ...
    grid_divides_atlas = atlas_width % columns == 0 and atlas_height % rows == 0
    exact_cell = (
        {"width": atlas_width // columns, "height": atlas_height // rows}
        if grid_divides_atlas
        else None
    )
    ...
    payload = {
        ...
        "atlas": {
            ...
            "grid_divides_atlas": grid_divides_atlas,
            "cell_width_exact": exact_cell["width"] if exact_cell else None,
            "cell_height_exact": exact_cell["height"] if exact_cell else None,
        ...
        "ue_notes": {
            ...
            "frame_order": "row-major-left-to-right-top-to-bottom",
            "grid_divides_atlas": grid_divides_atlas,
            "sub_uv_direct_ready": grid_divides_atlas
            and is_power_of_two(atlas_width)
            and is_power_of_two(atlas_height),
            "sub_uv_risk": None
            if grid_divides_atlas
            else "atlas dimensions do not divide evenly by grid; direct UE SubUV can jitter or sample offset",
            "recommended_end_frame": max(0, len(normalized_paths) - 1),
        },
    }
...
def add_recommendation_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--atlas-size-mode",
        choices=("nearest-power-of-two", "pad-to-power-of-two", "raw"),
        default="pad-to-power-of-two",
        help="Atlas output size policy. Default pads each atlas axis up to a power of two without rescaling cell content.",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    build.add_argument(
        ...
        default="contain",
        help="Frame fit policy. `contain` preserves the full source frame with padding, `cover` crops to fill, `stretch` distorts to fit.",
    )
    ...
    build.add_argument(
        "--atlas-name",
        default="",
        help="Optional atlas filename override. Defaults to T_<Descriptor>_<Grid>_VFX.png, for example T_DustFall_12x12_VFX.png.",
    )
