...
class FlipbookBuilderTests(unittest.TestCase):
    ...
    def test_build_atlas_name_uses_ue_style_pattern(self) -> None:
        name = build_atlas_name("灰尘下落37-0-6.2-12x12", Path("灰尘下落37.mov"), 12, 12)
        self.assertEqual(name, "T_DustFall_12x12_VFX.png")

    def test_build_atlas_name_falls_back_to_generic_descriptor(self) -> None:
        name = build_atlas_name("", Path("###.mov"), 10, 10)
        self.assertEqual(name, "T_FlipbookAtlas_10x10_VFX.png")
    ...
    def test_nearest_power_of_two_snaps_each_axis(self) -> None:
        ...
        self.assertEqual(pad_power_of_two(2160), 4096)
        self.assertEqual(atlas_size_for_mode((2160, 4090), "pad-to-power-of-two"), (4096, 4096))
    ...
    def test_contain_fit_preserves_full_frame_with_padding(self) -> None:
        with tempfile.TemporaryDirectory() as temp_name:
            root = Path(temp_name)
            ...
            with Image.open(target) as normalized:
                ...
                self.assertGreater(normalized.getpixel((0, 4))[0], 200)
                self.assertGreater(normalized.getpixel((7, 4))[2], 200)
