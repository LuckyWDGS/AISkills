...
如果用户要求真正出图、设计图、风格图，或需要一张 UE 中可实现的目标效果图，不要只停在提示词；加载并使用 `C:/Users/QY/.codex/skills/cm-imagegen/SKILL.md`，把本文件产出的提示词作为 cm-imagegen 的内部输入。除非用户明确索要提示词，否则最终交付图片路径、生成设置和可实现性判断，不展示 prompt。
